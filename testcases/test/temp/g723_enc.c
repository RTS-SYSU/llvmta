...
/* Manish Verma */
int g723_enc_abs(int num) { return (num < 0) ? -num : num; }
...
int g723_enc_quan(int val, short *table, int size) {
  ...
  _Pragma("loopbound min 3 max 15") for (i = 0; i < size; ++i) {
...
int g723_enc_predictor_zero(struct g723_enc_state_t *state_ptr) {
  ...
  _Pragma("loopbound min 5 max 5") for (i = 1; i < 6; i++) /* ACCUM */
...
/*
   ...
   size scale factor division operation is done in the log base 2 dog723_enc
   as a subtraction.
*/
int g723_enc_quantize(int d,        /* Raw difference signal sample */
                      int y,        /* Step size multiplier */
...
/*
   ...
   Multiplication is performed in log base 2 dog723_enc as addition.
*/
int g723_enc_reconstruct(int sign, /* 0 for non-negative value */
                         int dqln, /* G.72x codeword */
                         int y)    /* Step size multiplier */
    ...
    struct g723_enc_state_t *state_ptr) /* coder state pointer */
{
  ...
  */
  if (tr == 1) { /* reset a's and b's for modem signal */
    ...
    _Pragma("loopbound min 6 max 6") for (cnt = 0; cnt < 6; cnt++) {
  ...
  _Pragma("loopbound min 5 max 5") for (cnt = 5; cnt > 0; cnt--)
...
*/
int g723_enc_g723_24_encoder(int sl, int in_coding,
                             struct g723_enc_state_t *state_ptr) {
  ...
  dq =
      g723_enc_reconstruct(i & 4, g723_enc_dqlntab[i], y); /* quantized diff. */
...
void g723_enc_init_state(struct g723_enc_state_t *state_ptr) {
  ...
  _Pragma("loopbound min 2 max 2") for (cnta = 0; cnta < 2; cnta++) {
    state_ptr->a[cnta] = 0;
    state_ptr->pk[cnta] = 0;
    state_ptr->sr[cnta] = 32;
  }
  _Pragma("loopbound min 6 max 6") for (cnta = 0; cnta < 6; cnta++) {
    state_ptr->b[cnta] = 0;
...
void g723_enc_init() {
  ...
  _Pragma("loopbound min 256 max 256") for (i = 0; i < 256; i++)
      g723_enc_INPUT[i] += x;
}
...
int g723_enc_return() {
  ...
  _Pragma("loopbound min 256 max 256") for (i = 0; i < 256; i++) check_sum +=
      g723_enc_OUTPUT[i];
...
void _Pragma("entrypoint") g723_enc_g723_enc() {
  ...
  _Pragma("loopbound min 0 max 0") while (resid) resid =
      g723_enc_pack_output(0, enc_bits);
}

void g723_enc(void *n) {
  g723_enc_init();
  g723_enc_g723_enc();
