...
// unsigned int inputLen;                     /* length of input block */
{
  ...
  if (inputLen >= partLen) {
    ...
    _Pragma("loopbound min 0 max 0") for (i = partLen; i + 63 < inputLen;
                                          i += 64)
...
void md5_memset(POINTER output, int value, unsigned int len) {
  ...
  _Pragma("loopbound min 128 max 208") for (i = 0; i < len; i++)(
...
void md5_encode(unsigned char *output, UINT4 *input, int len) {
  ...
  _Pragma("loopbound min 2 max 16") for (j = 0; j < len; j += 4) {
...
void md5_decode(UINT4 *output, unsigned char *input, unsigned int len) {
  ...
  _Pragma("loopbound min 16 max 16") for (i = 0, j = 0; j < len; i++, j += 4) {
...
void md5_memcpy(POINTER output, POINTER input, unsigned int len) {
  ...
  _Pragma("loopbound min 0 max 55") for (i = 0; i < len; i++) output[i] =
      input[i];
}
...
void md5_memset_x(unsigned char *ptr, int value, unsigned long len) {
  _Pragma("loopbound min 16 max 64") while (len--) *ptr++ = value;
...
// */ unsigned char *block;                          /* block of values to mix
// in */ unsigned int blockLen;                                   /* length of
// block */
{
  MD5_CTX context;
  ...
  _Pragma("loopbound min 16 max 16") for (i = 0; i < 16; i++) {
...
void md5_InitRandomStruct(R_RANDOM_STRUCT *randomStruct) {
  ...
  */
  _Pragma("loopbound min 256 max 256") while (1) {
...
// */ R_RANDOM_STRUCT *randomStruct;                          /* random
// structure */
{
...
int md5_return(void) { return md5_bytesNeeded; }

void _Pragma("entrypoint") md5_main(void) {
  R_RANDOM_STRUCT randomStruct;
  R_RANDOM_STRUCT randomStruct2;
  /* We first generate parameters, and then do some key exchange each followed
   * by a key computation...*/
  int keys_exchanged = 0;

  md5_InitRandomStruct(&randomStruct);

  _Pragma("loopbound min 10 max 10") while (keys_exchanged != EXCHANGEKEYS) {
    keys_exchanged++;
