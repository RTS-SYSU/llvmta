...
void adpcm_dec_decode( int input )
{
  ...
  /* adpcm_dec multiply accumulate loop for samples and coefficients */
...
void adpcm_dec_init()
{
  ...
  /* Since the number of times we loop in adpcm_dec_sin depends on the
     argument we add the fact: xxadpcm_dec_0:[  ]: */
...
void _Pragma( "entrypoint" ) adpcm_dec_adpcm_dec( void )
{
  int i;
