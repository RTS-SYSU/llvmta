...
int minver_mmul(int row_a, int col_a, int row_b, int col_b) {
  ...
  _Pragma("loopbound min 3 max 3") for (i = 0; i < row_c; i++) {
    _Pragma("loopbound min 3 max 3") for (j = 0; j < col_c; j++) {
      w = 0.0;
      _Pragma("loopbound min 3 max 3") for (k = 0; k < row_b; k++) w +=
          minver_a[i][k] * minver_b[k][j];
...
int minver_minver(int side, double eps) {
  ...
  if (side < 2 || side > 500 || eps <= 0.0)
    return (999);
  w1 = 1.0;
  _Pragma("loopbound min 3 max 3") for (i = 0; i < side; i++) work[i] = i;
  _Pragma("loopbound min 3 max 3") for (k = 0; k < side; k++) {
    wmax = 0.0;
    _Pragma("loopbound min 1 max 3") for (i = k; i < side; i++) {
    ...
    if (r != k) {
      ...
      work[k] = work[r];
      work[r] = iw;
      _Pragma("loopbound min 3 max 3") for (j = 0; j < side; j++) {
        w = minver_a[k][j];
        minver_a[k][j] = minver_a[r][j];
        minver_a[r][j] = w;
      }
    }
    _Pragma("loopbound min 3 max 3") for (i = 0; i < side; i++)
        minver_a[k][i] /= pivot;
    _Pragma("loopbound min 3 max 3") for (i = 0; i < side; i++) {
      if (i != k) {
        w = minver_a[i][k];
  ...
  _Pragma("loopbound min 3 max 3") for (i = 0; i < side;) {
    _Pragma("loopbound min 1 max 3") while (1) {
      k = work[i];
      if (k == i)
        break;
      iw = work[k];
...
void minver_init() {
  ...
  _Pragma("loopbound min 3 max 3") for (i = 0; i < 3; i++) {
    _Pragma("loopbound min 3 max 3") for (j = 0; j < 3; j++) minver_a[i][j] +=
        x;
  }
}
...
int minver_return() {
  ...
  _Pragma("loopbound min 3 max 3") for (i = 0; i < 3; i++) {
    _Pragma("loopbound min 3 max 3") for (j = 0; j < 3; j++) check_sum +=
        minver_a_i[i][j];
  }
...
void _Pragma("entrypoint") minver_main() {
  ...
  eps = 1.0e-6;
  _Pragma("loopbound min 3 max 3") for (i = 0; i < 3; i++) {
    _Pragma("loopbound min 3 max 3") for (j = 0; j < 3; j++) minver_aa[i][j] =
        minver_a[i][j];
  }

  minver_minver(3, eps);
  _Pragma("loopbound min 3 max 3") for (i = 0; i < 3; i++) {
    _Pragma("loopbound min 3 max 3") for (j = 0; j < 3; j++) minver_a_i[i][j] =
        minver_a[i][j];
  }
