...
*/
void countnegative_initSeed(void) { countnegative_seed = 0; }
...
void countnegative_initialize(matrix Array) {
  ...
  _Pragma("loopbound min 20 max 20") for (OuterIndex = 0; OuterIndex < MAXSIZE;
                                          OuterIndex++)
      _Pragma("loopbound min 20 max 20") for (InnerIndex = 0;
                                              InnerIndex < MAXSIZE;
                                              InnerIndex++)
...
void countnegative_sum(matrix Array) {
  ...
  _Pragma("loopbound min 20 max 20") for (Outer = 0; Outer < MAXSIZE; Outer++)
      _Pragma(
