...
void jfdctint_init() {
  ...
  _Pragma("loopbound min 64 max 64") for (i = 0; i < 64; i++) {
...
int jfdctint_return() {
  int checksum = 0;
  int i;
  _Pragma("loopbound min 64 max 64") for (i = 0; i < 64; ++i) checksum +=
      jfdctint_data[i];
...
void jfdctint_jpeg_fdct_islow(void) {
  ...
  _Pragma("loopbound min 8 max 8") for (ctr = DCTSIZE - 1; ctr >= 0; ctr--) {
    ...
    dataptr[2] =
        (int)DESCALE(z1 + tmp13 * FIX_0_765366865, CONST_BITS - PASS1_BITS);
    dataptr[6] =
        (int)DESCALE(z1 + tmp12 * (-FIX_1_847759065), CONST_BITS - PASS1_BITS);
...
*/
void _Pragma("entrypoint") jfdctint_main(void) { jfdctint_jpeg_fdct_islow(); }
void jfdctint(void *n) {
  jfdctint_init();
  jfdctint_main();
  jfdctint_return();
}
