...
//   int j = 0;
//   int sum = 0;
//   _Pragma("loopbound min 10 max 10") for (; i < 10; ++i)
//       _Pragma("loopbound min 12 max 12") for (j = 0; j < 12; ++j) {
//     sum = i + j;
//   }
//   return sum;
// }
...
//   int i = 0;
//   int sum = 0;
//   _Pragma("loopbound min 10 max 10") for (; i < 10; ++i) { sum += i; }
//   return sum;
// }
// int myloop3() {
//   int i = 0;
...
void ndes_init() {
  ...
  static volatile char ndes_ipc2_tmp[49] = {
      0,  14, 17, 11, 24, 1,  5,  3,  28, 15, 6,  21, 10, 23, 19, 12, 4,
      26, 8,  16, 7,  27, 20, 13, 2,  41, 52, 31, 37, 47, 55, 30, 40, 51,
      45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32};
  _Pragma("loopbound min 57 max 57") for (i = 0; i < 57; i++) ndes_ipc1[i] =
      ndes_ipc1_tmp[i];
  _Pragma("loopbound min 49 max 49") for (i = 0; i < 49; i++) ndes_ipc2[i] =
      ndes_ipc2_tmp[i];
...
void ndes_des(ndes_immense inp, ndes_immense key, int *newkey, int isw,
  ...
  if (initflag) {
    ...
    _Pragma("loopbound min 31 max 31") for (j = 2; j <= 32; j++) ndes_bit[j] =
        (shifter <<= 1);
  }
  ...
  if (*newkey) {
    *newkey = 0;
    ...
    _Pragma("loopbound min 28 max 28") for (j = 28, k = 56; j >= 1; j--, k--) {
    ...
    _Pragma("loopbound min 16 max 16") for (i = 1; i <= 16; i++) {
      pg = kns[i];
      ndes_ks(/* key,*/ i, &pg);
      kns[i] = pg;
    }
  ...
  _Pragma("loopbound min 32 max 32") for (j = 32, k = 64; j >= 1; j--, k--) {
    itmp.r = itmp.r << 1;
...
void ndes_cyfun(unsigned long ir, ndes_great k, unsigned long *iout) {
  static volatile long iet[49] = {
      0,  32, 1,  2,  3,  4,  5,  4,  5,  6,  7,  8,  9,  8,  9,  10, 11,
  ...
  volatile char ibin[16] = {0, 8, 4, 12, 2, 10, 6, 14,
                            1, 9, 5, 13, 3, 11, 7, 15};
  ndes_great ie;
  ...
  _Pragma("loopbound min 16 max 16") for (j = 16, l = 32, m = 48; j >= 1;
                                          j--, l--, m--) {
  ...
  _Pragma("loopbound min 4 max 4") for (j = 1, m = 5; j <= 4; j++, m++) {
    iec[j] = ietmp1 & 0x3fL;
    iec[m] = ietmp2 & 0x3fL;
    ietmp1 >>= 6;
    ietmp2 >>= 6;
  ...
  _Pragma("loopbound min 8 max 8") for (jj = 8; jj >= 1; jj--) {
    j = iec[jj];
  ...
  *iout = 0L;
  ...
  _Pragma("loopbound min 32 max 32") for (j = 32; j >= 1; j--) *iout =
      (*iout << 1);
...
void ndes_ks(/*ndes_immense key, */ int n, ndes_great *kn) {
  ...
  if (n == 1 || n == 2 || n == 9 || n == 16) {
    ...
    _Pragma("loopbound min 2 max 2") for (i = 1; i <= 2; i++) {
      ndes_icd.r = (ndes_icd.r | ((ndes_icd.r & 1L) << 28)) >> 1;
      ndes_icd.l = (ndes_icd.l | ((ndes_icd.l & 1L) << 28)) >> 1;
    }
  }
  ...
  _Pragma("loopbound min 16 max 16") for (j = 16, k = 32, l = 48; j >= 1;
                                          j--, k--, l--) {
    (*kn).r = (*kn).r << 1;
    (*kn).r =
        ((*kn).r) | (unsigned short)ndes_getbit(ndes_icd, ndes_ipc2[j], 28);
    (*kn).c = (*kn).c << 1;
    (*kn).c =
        ((*kn).c) | (unsigned short)ndes_getbit(ndes_icd, ndes_ipc2[k], 28);
    (*kn).l = (*kn).l << 1;
    (*kn).l =
        ((*kn).l) | (unsigned short)ndes_getbit(ndes_icd, ndes_ipc2[l], 28);
  }
}

int ndes_return() { return (ndes_icd.r + ndes_icd.l + (-8390656)) != 0; }
